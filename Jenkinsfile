pipeline {
  ...
  environment {
    GIT_REPO_URL = 'git@github.com:zchelalo/neuraclinic-api-gateway.git'
  ...
  stages {
    ...
    stage('Go pipeline') {
      ...
      stages {
        ...
        stage('Build') {
          steps {
            sh 'mkdir -p dist && go build -buildvcs=false -trimpath -o dist/neuraclinic-api-gateway ./cmd'
          }
          post {
            always {
    ...
    stage('Docker Build') {
      steps {
        sh 'docker build -f .docker/Dockerfile -t neuraclinic-api-gateway:${BUILD_NUMBER} .'
      }
    }
        ...
        withSonarQubeEnv(credentialsId: 'sonar-token', installationName: 'SonarQube') {
          sh '''
            ${SONAR_SCANNER_HOME}/bin/sonar-scanner \
              -Dsonar.projectKey=neuraclinic-api-gateway \
              -Dsonar.sources=. \
              -Dsonar.exclusions=gen/**,dist/** \
